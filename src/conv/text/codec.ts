...
import { useCodecEditor } from '../../codec';
...
// export const useCodecEditor = (editorFunc: EditorFunc, codec: TextCodec, props: useCodecEditorProps) => {
...
export const useCodecEditor = (editorFunc: EditorFunc, codec: TextCodec, props: useCodecEditorProps) => {
    ...
    const { startingValue, onChange = () => {}, initialChange = true } = props;

    // Internal buffer (encoded version)
    const [editorBuffer, setEditorBuffer] = useState(decode(startingValue));
